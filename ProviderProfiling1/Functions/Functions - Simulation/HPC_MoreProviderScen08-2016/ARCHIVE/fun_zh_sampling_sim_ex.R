...
zh_sampling_ex <- function(provs,b.yi,n.perzh,funs,sims=10,seed=123,trim=0.02,
                            l.plots=F,ref="A",sav=T,plot.t=1:4,confs=10,cors=0){

#Determine coefficients for each scenario
#  b.coefs.ex  <- c(0,rep(1,provs-1)) #Coefficients for the multinom model with first provider as reference
#  coefs.out   <- c(1,seq(-1,1,length.out=provs-1)) #Coefficients for the outcome model. Takes values from seq based on length provs 
...
#Make list for chars of the samples
  ...
  zh.chars  <-  array(NA,dim=c(1,provs*2+1,sims),dimnames=list(1:1,
                      nams.cols,1:sims))
#Weight matrices
#  w.tab     <- array(NA,dim=c(1,9*2,sims),
#                   dimnames=list(1:1,
#                   rep(c("Min","Q1","Med","Mean","Q3","Max","trim.A","trim.B","trim.C"),2),1:sims)) #Untrimmed and trimmed are rbinded

#Matching char matrix
#  mat.char  <- array(NA,dim=c(1,2,sims),
#                   dimnames=list(1:1,c("calp","mat.set"),1:sims)) #Untrimmed and trimmed are rbinded
...
#Loop over list elements to collect results  
  for (jj in 1:length(funs)){
    ...
    tot.bias[jj,]    <- t(apply(zh.coef[[jj]],c(1,2),mean)-matrix(coefs.out[-1]))
    
    #Percentage bias (WATCH OUT FOR SITUATION WHERE TRUE COEF IS 0...)
    per.bias[jj,]    <- t(100*tot.bias[jj,]/matrix(coefs.out[-1]))
    ...
    cov.mat[jj,]     <- apply(zh.ci[[jj]][,1,]<= coefs.out[-1] & coefs.out[-1] <= zh.ci[[jj]][,2,],1,mean)
